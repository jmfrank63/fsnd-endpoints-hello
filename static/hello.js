...
/*
 * Execute a greet by period message taking a string such as
 * "Good morning" or "Good night" and a name and returns the
 * two arguments as "Period" "Name"
 */
 function greetByPeriod() {
	 // Get the period from the period_field element
	 var period = document.getElementById("period_field").value;
	 // Get the name from the name_field element
	 var name = document.getElementById("name_field").value;

	 // Call the greetByPeriod() function.
	 // It takes two arguments, the period and the name
	 // On success, the response is passed to the sayHelloCallback()
